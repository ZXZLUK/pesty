...
### 智能
**拼音搜索**（全拼/声母/多音字，"粘贴板"用 `ztb` 就能搜到）；自动识别文本/富文本/链接/图片/文件/颜色六种类型并按来源应用着色；右上角 `Podcast Agent` 可按需开启，把字幕或长文本编译为快速理解、平实口播、轻幽默口播或 20 条判断；密码管理器复制自动排除；多显示器下面板跟随鼠标所在屏幕。
...
- `Podcast Agent` 关闭时不产生模型调用；开启后，仅命中的字幕/长文本会交给本地编译器，并由编译器发送至其配置的 MiniMax 模型服务
