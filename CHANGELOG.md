...
- **新增：鼠标顶边唤出（热边触发）**：每块屏幕最顶边贴一条隐形 4pt 触发条（窗口级别在菜单栏之上，全空间可见），鼠标碰到最顶上的线停留 0.25 秒即召唤面板。防误触：停留式触发（快速甩过去够菜单栏不触发）；单次武装（触发过一次后光标不离开顶边不会重弹，收起后原地不动也不会立即重弹）。设置 → 行为 →「鼠标碰屏幕顶边唤出」可关。代价：屏幕最顶部 4pt 区域的点击会被触发条吞掉（菜单栏图标垂直居中，实际不受影响）。显示器增减/唤醒时自动重建。
