...
- **修复：抓取序列改用 Timer 步进**——此前抓取按键用 `DispatchQueue.main.asyncAfter` 调度，实测在本应用主队列繁忙时**整批按键经常从未执行**（轨迹日志证实调度块从未进入）。改为 0.05s repeating Timer 步进状态机：逐步检查前台身份、到点即发、前台切换即中止。与全系统输入监听一样遵循"边缘触发类机制一律 Timer 轮询、禁用 asyncAfter"的应用内纪律。
- **改进：顶边召唤加 0.2 秒停留**——零停留时鼠标擦过顶边中段会立即误弹（用户实际感受到面板"跳出来"）。现在需要碰到顶边并停留 0.2 秒才召唤；路过擦边自动取消。有意的召唤（停一下）不受影响。QuickNote 底边维持 0.3 秒停留。停留时长均为可调常量。
